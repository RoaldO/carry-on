...
async def activate_account(request: ActivateRequest) -> dict:
    ...
    if user.get("activated_at"):
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail="Account already activated"
        )
...
async def login(request: LoginRequest) -> dict:
    ...
    if not user.get("activated_at"):
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail="Account not activated"
        )
...
async def update_password(request: UpdatePasswordRequest) -> dict:
    ...
    if not verify_password_hash(request.current_password, stored_hash):
        raise HTTPException(
            status_code=HTTP_401_UNAUTHORIZED, detail="Invalid current password"
        )

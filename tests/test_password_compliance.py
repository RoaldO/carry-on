...
from carry_on.api import pin_security
...
class TestActivateRequestValidation:
    ...
    def test_accepts_long_password(self):
        ...
        long_pw = "a" * 200
        req = pin_security.ActivateRequest(email="test@example.com", password=long_pw)
...
class TestLoginRequestValidation:
    ...
    def test_rejects_password_under_4_chars(self):
        ...
        with pytest.raises(ValidationError):
            pin_security.LoginRequest(email="test@example.com", password="123")

    def test_accepts_long_password(self):
        """LoginRequest should accept very long passwords (no max)."""
        long_pw = "a" * 200
        req = pin_security.LoginRequest(email="test@example.com", password=long_pw)
        assert len(req.password) == 200
